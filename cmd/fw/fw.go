...
var (
	...
	cooldownDuration   = time.Millisecond * 200
)

var fws = []*FW{
	{
		name:  "go main",
		files: []string{"cmd/test/main.go"},
		run: func() RunFunc {
			var (
				cmd    *exec.Cmd
				ctx    context.Context
	...
	},
	{
		...
		name:       "sqlc generate",
		regexMatch: regexp.MustCompile(`sql\/.*.sql$`),
	...
	},
	{
		...
		name:       "templ",
		regexMatch: regexp.MustCompile(`view\/.*.templ$`),
		run: func(e fsnotify.Event) {
	...
	},
	{
		...
		name:       "brotli",
		regexMatch: regexp.MustCompile(`browser\/.*.js$`),
		run:        brotliFunc(),
	},
	{
...
func esbuildFunc() RunFunc {
	opts := api.BuildOptions{
		...
		Outfile:           "browser/out.js",
		Plugins: []api.Plugin{{
			Name: "end",
			Setup: func(build api.PluginBuild) {
				build.OnEnd(func(result *api.BuildResult) (api.OnEndResult, error) {
					fmt.Println("esbuild end build")
...
func brotliFunc() RunFunc {
	return func(e fsnotify.Event) {
		...
		outFile, err := os.Create(e.Name + ".br")
		if err != nil {
